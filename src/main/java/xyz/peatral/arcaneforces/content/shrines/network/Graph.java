...
public class Graph<T> {
    private Map<T, Set<T>> adjacency = new LinkedHashMap<>();
    ...
    public Graph(Map<T, Set<T>> adjacency, BiFunction<T, T, Double> distanceFunction) {
        this.adjacency = new LinkedHashMap<>(adjacency);
        this.distanceFunction = distanceFunction;
    }


    public boolean addNode(T node) {
        return adjacency.put(node, new HashSet<>()) != null;
    }

    public boolean addAndUpdateNeighbors(T node, BiPredicate<T, T> neighborPredicate) {
        boolean modified = addNode(node);
        //if (!modified) {
        //    return false;
        //}
    ...
    public boolean setNeighbors(T node, Set<T> neighbors) {
        ...
        return adjacency.put(node, neighbors) != null;
    }
    ...
    public boolean remove(T node) {
        boolean modified = adjacency.remove(node) != null;
        if (!modified) {
            return false;
        }
        for (T otherNode : adjacency.keySet()) {
